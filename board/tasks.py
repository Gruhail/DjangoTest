...
def respond_send_email(respond_id):
    ...
    send_mail(
        subject=f'GuildBoard: поступил отклик на объявление',
        message=f'Добрый день, {respond.post.author},  На ваше объявление есть новый отклик\n'
                f'Прочитать отклик:\nhttp://127.0.0.1:8000/responses/{respond.post.id}',
        from_email='Semensokolov2206@icloud.com',
...
def respond_accept_send_email(response_id):
    ...
    send_mail(
        subject=f'GuildBoard: ваш отклик принят',
...
def send_mail_monday_8am():
    ...
    if list_week_posts:
        for user in User.objects.filter():
            ...
            send_mail(
                subject=f'News: посты за прошедшую неделю.',
